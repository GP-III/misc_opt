...
# define TH08_window_style (WS_VISIBLE | WS_GROUP | WS_SYSMENU | WS_MAXIMIZEBOX) // 0x100A0000


extern void __board_write (HANDLE fHandle, BYTE DIK_set);
  ...
  case WM_COMMAND:
    ...
    case IDC_BUT_CONFIRM:
      ...
      __try 
      {
        ...
        __molios_bclr (gkeyedit.fHandle);
        __board_write (gkeyedit.fHandle, DIK_cache[CODE_FUNC_SWITCH]);
        __calc_write (gkeyedit.fHandle);
        b_alloc (gkeyedit.fHandle);
        __monitor_write (gkeyedit.fHandle);
